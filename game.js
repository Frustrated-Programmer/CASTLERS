...
var commands = [
    ...
    },//SHOP
    {
        ...
        waitTime: 0,
        does: function (words, message, player) {
			var getNumbers = function (message) {
                ...
                for (var i = 0; i < message.content.length; i++) {
                    ...
                    for (var j = 0; j < nums.length; j++) {
                        if (message.content[i] === nums[j]) {
                            if(!numWords.length){
                                numWords[0]="";
                            }
                            q=true;
                            numWords[numWord] += message.content[i];
                            f = true;
                        }
                    }
                    if (!f&&q) {
            ...
            var where = "idk";
            if(numWords.length){
                if(parseInt(numWords[0],10)!==0) {
                    where = parseInt(words[0], 10);
                    where--
                }
                if(numWords[1]){
                    if(numWords[1]!==0){
                        amount = parseInt(numWords[1],10);
                    }
                }
                else {
    ...
    },//SPY
    {
        ...
        does: function (words, message, player) {
            ...
            if(words[1]){
                ...
                message.channel.send("You must include an attackNum with it");
            }
        }
    },//ATTACK
